      ...
      atom.workspace.observeTextEditors(editor => {
        ...
        editorSubscriptions.add(
          editor.onDidChange(() => {
            ...
            const cells = store.markers.cells;
            const regexString = codeManager.getRegexString(editor);

            const regex = new RegExp(regexString, "g");

            buffer.scanInRange(regex, new Range([0, 0], end), ({ range }) => {
              for (let cell of cells.values()) {
                if (cell.getBufferRange().isEqual(range) && cell.isValid()) {
                  return;
                } else if (range.containsRange(cell.getBufferRange())) {
                  store.markers.deleteCell(cell.id);
                }
              }
              const cellHeader = editor.markBufferRange(range, {
                invalidate: "touch"
              });
              editor.decorateMarker(cellHeader, {
                type: "text",
                style: { color: "cyan" }
              });
              store.markers.newCell(cellHeader);
            });
          })
        );
